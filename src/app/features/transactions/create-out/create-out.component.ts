...
@Component({
  ...
  template: `
    <div class="create-transaction">
      <app-card title="Create Transfer OUT Transaction">
        <form (ngSubmit)="onSubmit()" class="transaction-form">
          <app-alert variant="info">
            Transfer OUT removes stock from inventory.
            {{ authService.canOverridePrices() ?
               'As Admin/Supervisor, you can override selling prices.' :
               'Selling prices will be taken from item price list.' }}
          </app-alert>
              ...
              <div class="line-fields">
                <div class="form-row">
                  ...
                  <app-input
                    ...
                    hint="Available: {{ line.availableStock ?? '...' }}"
                  ></app-input>
                </div>

                <div class="form-row" *ngIf="authService.canOverridePrices()">
                  <app-input
                    ...
                    type="number"
                    placeholder="0.00"
                    hint="Leave empty to use item price list"
                  ></app-input>
                ...
                <app-input
                  *ngIf="authService.canOverridePrices() && line.unitPrice"
                  [(ngModel)]="line.priceNotes"
                  [name]="'priceNotes_' + i"
                  label="Price Override Reason *"
                  placeholder="e.g., Manager discount, Promotional sale"
                  [required]="true"
                ></app-input>

                <app-alert *ngIf="!authService.canOverridePrices()" variant="info">
                  Price will be taken from item price list. Contact supervisor to override.
                </app-alert>
              </div>
            </div>
          </div>
  ...
  `,
  styleUrls: ['./create-out.component.scss']
})
export class CreateOutComponent implements OnInit {
  ...
  getStockError(line: TransactionLine): string {
  ...
  isFormValid(): boolean {
    ...
    return this.lines().every(line => {
      const hasValidFields = line.itemCode.trim() !== '' && line.quantity > 0;
      const hasValidStock = line.availableStock === undefined || line.quantity <= line.availableStock;
      const hasValidPriceNotes = !line.unitPrice || (line.priceNotes && line.priceNotes.trim() !== '');

      return hasValidFields && hasValidStock && hasValidPriceNotes;
    });
  }
  ...
  private executeSubmit(): void {
    ...
    this.transactionsService.createOutTransaction(request).subscribe({
      next: (transaction) => {
        this.toastService.success('Transfer OUT transaction created successfully');
  ...
  commitTransaction(transactionId: string): void {
    this.transactionsService.commitTransaction(transactionId).subscribe({
      ...
      error: (err) => {
        ...
        this.toastService.warning('Transaction created but failed to commit. View in transaction list.');

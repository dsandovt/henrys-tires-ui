...
export class ItemsService {
  private readonly API_URL = `${environment.apiUrl}/v1/item`;

  constructor(private http: HttpClient) {}

  getItems(
    params?: PaginationParams & { classification?: string },
    ...
    if (params?.classification)
      httpParams = httpParams.set('classification', params.classification);

    return this.http
      .get<{ success: boolean; data: any }>(this.API_URL, { params: httpParams })
      .pipe(
        map((response) => ({
  ...
  getItemById(id: string): Observable<Item> {
    return this.http
      .get<{ success: boolean; data: Item }>(`${this.API_URL}/${id}`)
  ...
  getAllItems(): Observable<Item[]> {
    return this.http
      .get<{ success: boolean; data: Item[] }>(`${this.API_URL}/all`)
      .pipe(map((response) => response.data));
  }
  ...
  createItem(request: CreateItemRequest): Observable<Item> {
    return this.http
      .post<{ success: boolean; data: Item }>(this.API_URL, request)
      .pipe(map((response) => response.data));
  }
